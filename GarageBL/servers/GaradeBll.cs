...
namespace GarageBL.servers
{
    public class GaradeBll : IGaradeBll
    {
        ...
        private readonly ILogger<GaradeBll> _logger;

        private readonly IMapper _mapper;
        private readonly IHttpClientFactory _httpClientFactory;

        public GaradeBll(IGarageDb garageDb, IMapper mapper, IHttpClientFactory httpClientFactory, ILogger<GaradeBll> logger)
        {
            _logger = logger;
            _garageDb = garageDb;
            _mapper = mapper;
        ...
        public async Task<List<Garage>> FetchAndSaveFromApiAsync()
        {
            try
            {
                ...
                foreach (var record in records.EnumerateArray())
                {
                    ...
                    {
                        _logger.LogError(ex, "Failed to parse garage record from API: {Record}", record.ToString());
                    }
                }
            ...
            {
                _logger.LogError(ex, "Error in FetchAndSaveFromApiAsync");
                throw;
            }
        }
